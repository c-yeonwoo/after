...
//
// 원칙 — 메일이 곧 다음 행동이다.
//   · 제목에 무슨 일이 일어났는지를 넣는다. "이클립스 알림" 같은 제목은 안 연다.
...
/*
  이름은 하나다. 예전에는 정식 `애프터선셋` / 본문용 `애프터` 로 나눠 썼는데,
  6음절이 문장 안에서 장황해서였다. `이클립스` 는 그럴 이유가 없다.

  src/lib/brand.ts 를 가져오지 않고 여기 상수로 둔다 — Edge Function 은 Deno 로
  따로 번들되어 앱 소스를 참조하지 않는다. 이름을 바꿀 때 두 곳을 함께 고쳐야 한다.
*/
const BRAND = "이클립스";

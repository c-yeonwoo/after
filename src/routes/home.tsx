...
function HomePage() {
  ...
  const headline = meeting?.confirmed_at
    ? "만남이 잡혔어요."
...
function CandidatePreview({ candidate, isMale }: { candidate: Profile; isMale: boolean }) {
  return (
    <div className="overflow-hidden rounded-surface border border-border bg-card shadow-card">
      <div className="bg-gradient-to-br from-accent/40 via-card to-card px-5 pt-5 pb-6">
        ...
        <p className="headline mt-3 text-2xl">
          {candidate.name}
          {candidate.birth ? (
            <span className="ml-2 text-base text-muted-foreground">{ageFrom(candidate.birth)}</span>
          ) : null}
        </p>
